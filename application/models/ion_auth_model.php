...
*/
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email= false)
	{
	...
	{   // this is only for my particular case
            $searchable_fields = array ('u.username','u.email','ud.first_name','ud.last_name','ud.company',
                                        'ud.company_tax_id','ud.phone','ud.fax','ud.address1','ud.address2',
                                        'ud.city','ud.state','ud.zip_code','ud.dob','ud.website','g.name');
            $criterias = array ();

            foreach ($searchable_fields as $field) $criterias[$field]=$keyword;

            return $this->db->select ('u.id, ud.first_name, ud.last_name, u.email, u.active, g.name AS `group`')
                            ->join ($this->tables['meta'].' ud', 'ud.user_id=u.id')
                            ->join ($this->tables['groups'].' g', 'g.id=u.group_id')
                            ->from ($this->tables['users'].' u')->or_like($criterias)->distinct()->get();
	}
	 ...
	 * @author Camilo
	 **/
	public function search_vendors($keyword)
	{   // this is only for my particular case
            $searchable_fields = array ('u.username','u.email','ud.first_name','ud.last_name','ud.company',
                                        'ud.company_tax_id','ud.phone','ud.fax','ud.address1','ud.address2',
                                        'ud.city','ud.state','ud.zip_code','ud.dob','ud.website','g.name');
            ...
            return $this->db->select ('u.id, ud.first_name, ud.last_name, u.email, ud.company, u.active, g.name AS `group`')
                            ->join ($this->tables['meta'].' ud', 'ud.user_id=u.id')
                            ->join ($this->tables['groups'].' g', 'g.id=u.group_id')
                            ->from ($this->tables['users'].' u')
                            ->where (array('g.name'=>'vendor'))->or_like($criterias)->distinct()->get();
	}
	 ...
	 * @author Camilo
	 **/
	public function search_sellers($keyword)
	{   // this is only for my particular case
            $searchable_fields = array ('u.username','u.email','ud.first_name','ud.last_name','ud.company',
                                        'ud.company_tax_id','ud.phone','ud.fax','ud.address1','ud.address2',
                                        'ud.city','ud.state','ud.zip_code','ud.dob','ud.website','g.name');
            ...
            return $this->db->select ('u.id, ud.first_name, ud.last_name, u.email, ud.company, u.active, g.name AS `group`')
                            ...
                            ->join ($this->tables['groups'].' g', 'g.id=u.group_id')
                            ->from ($this->tables['users'].' u')
                            ->where (array('g.name'=>'seller'))->or_like($criterias)->distinct()->get();
	}
