...
class Ion_auth extends CI_Controller
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email= false) //need to test email activation
	{
		$email_activation = $this->ci->config->item('email_activation', 'ion_auth');

		if (!$email_activation)
		{
			$id = $this->ci->ion_auth_model->register($username, $password, $email);
			if ($id !== FALSE)
			{
				$this->set_message('account_creation_successful');
				$data = array();
				$message = $this->ci->load->view($this->ci->config->item('email_templates', 'ion_auth').'thanks.tpl.php', $data, true);
				
				$this->ci->email->clear();
				$config['mailtype'] = $this->ci->config->item('email_type', 'ion_auth');
				$this->ci->email->initialize($config);
				$this->ci->email->set_newline("\r\n");
				$this->ci->email->from($this->ci->config->item('admin_email', 'ion_auth'), 'feeder');
	 ...
	 * @authr Camilo
	 **/
	public function get_new_users_array($group_name=false)
	{
		return $this->ci->ion_auth_model->get_new_users($group_name)->result_array();
	}
	...
	{
		return $this->ci->ion_auth_model->search_users($keyword)->result_array();
	}
